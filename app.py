...
naver_txt = random.choice([
    f"**네이버 (포털 검색량)**: {target_item}의 상시 검색 하한선이 지속적으로 상승하며 대중적 인지도 확보.",
    f"**네이버 (포털 검색량)**: 검색 의도가 '정보 탐색'에서 '구매처 확인'으로 구체화되는 양상을 보임.",
    f"**네이버 (포털 검색량)**: 경쟁 브랜드 대비 검색 키워드의 다양성이 확보되어 노출 기회가 증가함."
])
google_txt = random.choice([
    f"**구글 (디지털 관심도)**: 핵심 타겟층의 정보 탐색이 능동적으로 발생하고 있음.",
    f"**구글 (디지털 관심도)**: 광범위한 트렌드 지표에서 우위를 점하며 전국적인 인지도 확산 중.",
    f"**구글 (디지털 관심도)**: 연관 동영상 및 리뷰 콘텐츠 소비가 활발해지며 브랜드 깊이가 강화됨."
])
insta_txt = random.choice([
    f"**인스타그램 (바이럴)**: 참여형 팬덤의 화력이 동종 상품군 대비 월등히 높음.",
    f"**인스타그램 (바이럴)**: 비주얼 중심의 콘텐츠 생산이 활발하여 브랜드 이미지가 고급화되는 추세.",
    f"**인스타그램 (바이럴)**: MZ세대의 '해시태그' 점유율이 급증하며 트렌드의 중심에 위치함."
])
